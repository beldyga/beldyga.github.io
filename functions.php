...
function getEncryptedPassword($password) {	
	return crypt($password, '$6$rounds=5000$' . ZIPSME_PASSWORD_SALT . '$');
}
...
function showLinkHistory() {
	...
	if (sizeof($links)==0) {
		...
		foreach ($links as $link) {
			echo '<tr>' . "\n";
				echo '<td><span class="link-prefix">' . SITE_URL . '</span>' . prepOutputText($link['url_name']) . '</td>' . "\n";
				echo '<td>' . $link['clicks'] . '</td>' . "\n";
				echo '<td><a href="admin.php?summary=' . $link['url_name'] . '">View Stats</a><span class="options-separator">|</span><a href="admin.php?edit=' . $link['url_name'] . '">Edit</a><span class="options-separator">|</span><a href="admin.php?pre_delete=' . $link['url_name'] . '">Delete</a>' . '</td>' . "\n";	
			echo '</tr>' . "\n";
		}
	}		
}
